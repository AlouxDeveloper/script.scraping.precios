...
def marcar_fallida(url, detalle=""):
    ...
    with open(CSV_ESTADO_URLS, 'a', newline='', encoding='utf-8') as f:
        ...
        writer.writerow([url, 'ERROR', detalle, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
...
def extraer_detalles_producto(product_url, writer):
    ...
    Devuelve True si el registro se escribió con éxito, False si la URL falló.
    """
...
def main():
    ...
    # 1b. Cargar progreso previo (éxitos + fallidas) para no repetir peticiones
    urls_procesadas = set()
    if os.path.exists(CSV_OUTPUT_PATH) and os.stat(CSV_OUTPUT_PATH).st_size > 0:
        try:
            with open(CSV_OUTPUT_PATH, 'r', newline='', encoding='utf-8') as f:
                lector_prev = csv.reader(f)
                next(lector_prev, None)  # cabecera
                urls_procesadas = {row[1] for row in lector_prev if len(row) > 1}
        except Exception as e:
            print(f"⚠️ Alerta leyendo avance previo: {e}")
    if os.path.exists(CSV_ESTADO_URLS) and os.stat(CSV_ESTADO_URLS).st_size > 0:
        try:
            with open(CSV_ESTADO_URLS, 'r', newline='', encoding='utf-8') as f:
                lector_fallidas = csv.reader(f)
                next(lector_fallidas, None)  # cabecera
                urls_procesadas |= {row[0] for row in lector_fallidas if row}
        except Exception as e:
            print(f"⚠️ Alerta leyendo URLs fallidas previas: {e}")
    print(f"📂 Avance: {len(urls_procesadas)} de {len(urls_a_procesar)} URLs ya procesadas.")
    ...
    try:
        with open(CSV_OUTPUT_PATH, 'a', newline='', encoding='utf-8') as csv_file:
            ...
            for url in tqdm(urls_a_procesar, desc="fahorro", unit="url", initial=len(urls_procesadas)):
                if url in urls_procesadas:
                    continue

                exito = extraer_detalles_producto(url, csv_writer)
                if not exito:
                    marcar_fallida(url)
