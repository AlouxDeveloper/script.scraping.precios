-- Calibracion de tau_alto, tau_bajo y delta_min (ALD-68), sobre el split
-- de calibracion (ALD-65) y el universo completo de int_candidatos_ndf
-- (ALD-72), con guardas activas (reciprocidad + magnitudes, ver
-- macros/pasa_guardas.sql). Por bucket de estilo de tienda (ALD-84: A
-- alto solape, B bajo solape / default), sobre v1/t1 -ALD-67 decidio no
-- correr el barrido vectorial de variantes de texto-. Corrida de
-- referencia 2026-09-22. Resultado y decision final anotados en el issue
-- de Linear; este archivo deja las consultas reproducibles.
--
-- Se corre a mano, desde la raiz del repo:
--   bq query --use_legacy_sql=false \
--     < transform/entity_resolution/sql/14_calibracion_umbrales_sanfer.sql

-- Paso 1: sweep de tau_alto por bucket. Objetivo de precision 0.95 (mismo
-- corte que ALD-58/ALD-83). Resultado: bucket A cruza en 0.05 (precision
-- 0.9500, n=526 positivos, 300 aceptados, cobertura 57.0%); bucket B cruza
-- en 0.04 (precision 0.9767, n=180 positivos, 43 aceptados, cobertura
-- 23.9% -bucket mucho mas delgado, intervalo de confianza mas ancho).
WITH bucket_tienda AS (
    SELECT tienda_slug,
        CASE
            WHEN tienda_slug IN (
                'gi', 'klyns', 'isseg', 'aurrera', 'heb', 'farmatodo',
                'guadalajara', 'benavides', 'walmart'
            ) THEN 'A'
            ELSE 'B'
        END AS bucket
    FROM `scenic-firefly-473823-f7.precios_gold.dim_tienda`
),

universo AS (
    SELECT
        split.producto_key,
        bucket_tienda.bucket,
        dp.ndf_id AS ndf_id_verdadero,
        UPPER(dn.laboratorio) = 'SANFER' AS es_positivo_sanfer
    FROM `scenic-firefly-473823-f7.precios_ml.producto_split_aportadores` AS split
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_producto` AS dp
        ON dp.producto_key = split.producto_key
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_ndf` AS dn
        ON dn.ndf_id = dp.ndf_id
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_tienda` AS dim_tienda
        ON dim_tienda.tienda_key = split.tienda_key
    INNER JOIN bucket_tienda
        ON bucket_tienda.tienda_slug = dim_tienda.tienda_slug
    WHERE split.split = 'calibracion'
),

candidato_top AS (
    SELECT
        producto_key,
        ndf_id AS ndf_id_candidato,
        distancia,
        margen,
        es_reciproco
        AND (atributos_producto.dosis_mg IS NULL OR atributos_ndf.dosis_mg IS NULL
             OR atributos_producto.dosis_mg = atributos_ndf.dosis_mg)
        AND (atributos_producto.volumen_ml IS NULL OR atributos_ndf.volumen_ml IS NULL
             OR atributos_producto.volumen_ml = atributos_ndf.volumen_ml)
        AND (atributos_producto.masa_g IS NULL OR atributos_ndf.masa_g IS NULL
             OR atributos_producto.masa_g = atributos_ndf.masa_g)
        AND (atributos_producto.piezas IS NULL OR atributos_ndf.piezas IS NULL
             OR atributos_producto.piezas = atributos_ndf.piezas) AS pasa_guardas
    FROM `scenic-firefly-473823-f7.precios_ml.int_candidatos_ndf`
    WHERE rank_desde_producto = 1
),

universo_top AS (
    SELECT
        universo.bucket,
        universo.es_positivo_sanfer,
        (candidato_top.ndf_id_candidato IS NOT NULL AND candidato_top.pasa_guardas)
            AS candidato_confiable,
        candidato_top.distancia,
        candidato_top.ndf_id_candidato = universo.ndf_id_verdadero AS top1_correcto
    FROM universo
    LEFT JOIN candidato_top USING (producto_key)
),

umbrales AS (
    SELECT ROUND(umbral, 3) AS umbral
    FROM UNNEST(GENERATE_ARRAY(0.0, 0.17, 0.005)) AS umbral
)

SELECT
    universo_top.bucket,
    umbrales.umbral AS tau_alto_candidato,
    COUNTIF(universo_top.es_positivo_sanfer) AS n_positivo,
    COUNTIF(
        universo_top.es_positivo_sanfer AND universo_top.candidato_confiable
        AND universo_top.distancia <= umbrales.umbral
    ) AS n_positivo_aceptado,
    SAFE_DIVIDE(
        COUNTIF(
            universo_top.es_positivo_sanfer AND universo_top.candidato_confiable
            AND universo_top.distancia <= umbrales.umbral AND universo_top.top1_correcto
        ),
        COUNTIF(
            universo_top.es_positivo_sanfer AND universo_top.candidato_confiable
            AND universo_top.distancia <= umbrales.umbral
        )
    ) AS precision,
    SAFE_DIVIDE(
        COUNTIF(
            universo_top.es_positivo_sanfer AND universo_top.candidato_confiable
            AND universo_top.distancia <= umbrales.umbral
        ),
        COUNTIF(universo_top.es_positivo_sanfer)
    ) AS cobertura_positivos
FROM universo_top
CROSS JOIN umbrales
GROUP BY universo_top.bucket, umbrales.umbral
ORDER BY universo_top.bucket, umbrales.umbral;

-- Paso 2: distribucion de margen entre positivos ya aceptados por
-- distancia (tau_alto elegido: A=0.05, B=0.04), separado por si el top1
-- fue correcto. Resultado: bucket A, aciertos (n=285) margen mediano
-- 0.0259 contra 0.0076 de los errores (n=15) -mas de 3x, la resta si
-- cancela el sesgo de estilo de tienda como esperaba el issue. Bucket B
-- tiene un solo error (n=1, insuficiente para una regla propia) -por eso
-- delta_min se decide GLOBAL, no por bucket.
WITH bucket_tienda AS (
    SELECT tienda_slug,
        CASE
            WHEN tienda_slug IN (
                'gi', 'klyns', 'isseg', 'aurrera', 'heb', 'farmatodo',
                'guadalajara', 'benavides', 'walmart'
            ) THEN 'A'
            ELSE 'B'
        END AS bucket
    FROM `scenic-firefly-473823-f7.precios_gold.dim_tienda`
),

universo AS (
    SELECT
        split.producto_key,
        bucket_tienda.bucket,
        dp.ndf_id AS ndf_id_verdadero,
        UPPER(dn.laboratorio) = 'SANFER' AS es_positivo_sanfer
    FROM `scenic-firefly-473823-f7.precios_ml.producto_split_aportadores` AS split
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_producto` AS dp
        ON dp.producto_key = split.producto_key
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_ndf` AS dn
        ON dn.ndf_id = dp.ndf_id
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_tienda` AS dim_tienda
        ON dim_tienda.tienda_key = split.tienda_key
    INNER JOIN bucket_tienda
        ON bucket_tienda.tienda_slug = dim_tienda.tienda_slug
    WHERE split.split = 'calibracion'
),

candidato_top AS (
    SELECT
        producto_key,
        ndf_id AS ndf_id_candidato,
        distancia,
        margen,
        es_reciproco
        AND (atributos_producto.dosis_mg IS NULL OR atributos_ndf.dosis_mg IS NULL
             OR atributos_producto.dosis_mg = atributos_ndf.dosis_mg)
        AND (atributos_producto.volumen_ml IS NULL OR atributos_ndf.volumen_ml IS NULL
             OR atributos_producto.volumen_ml = atributos_ndf.volumen_ml)
        AND (atributos_producto.masa_g IS NULL OR atributos_ndf.masa_g IS NULL
             OR atributos_producto.masa_g = atributos_ndf.masa_g)
        AND (atributos_producto.piezas IS NULL OR atributos_ndf.piezas IS NULL
             OR atributos_producto.piezas = atributos_ndf.piezas) AS pasa_guardas
    FROM `scenic-firefly-473823-f7.precios_ml.int_candidatos_ndf`
    WHERE rank_desde_producto = 1
),

aceptados_por_distancia AS (
    SELECT
        universo.bucket,
        candidato_top.margen,
        candidato_top.ndf_id_candidato = universo.ndf_id_verdadero AS top1_correcto
    FROM universo
    INNER JOIN candidato_top USING (producto_key)
    WHERE universo.es_positivo_sanfer
        AND candidato_top.ndf_id_candidato IS NOT NULL
        AND candidato_top.pasa_guardas
        AND (
            (universo.bucket = 'A' AND candidato_top.distancia <= 0.05)
            OR (universo.bucket = 'B' AND candidato_top.distancia <= 0.04)
        )
)

SELECT
    bucket,
    top1_correcto,
    COUNT(*) AS n,
    ROUND(AVG(margen), 5) AS margen_prom,
    APPROX_QUANTILES(margen, 4)[OFFSET(2)] AS margen_mediana
FROM aceptados_por_distancia
GROUP BY bucket, top1_correcto
ORDER BY bucket, top1_correcto;

-- Paso 3: sweep de delta_min GLOBAL (pooled A+B) sobre esos mismos
-- positivos aceptados por distancia. Resultado: 0.008 es el punto de
-- rendimientos decrecientes -atrapa 9 de 16 "confiados pero equivocados"
-- (56%) al costo de mandar 35 de 327 aciertos reales a banda gris
-- (10.7%); 0.01 no atrapa ni uno mas y ya cuesta 6 aciertos adicionales,
-- dominado por 0.008.
WITH bucket_tienda AS (
    SELECT tienda_slug,
        CASE
            WHEN tienda_slug IN (
                'gi', 'klyns', 'isseg', 'aurrera', 'heb', 'farmatodo',
                'guadalajara', 'benavides', 'walmart'
            ) THEN 'A'
            ELSE 'B'
        END AS bucket
    FROM `scenic-firefly-473823-f7.precios_gold.dim_tienda`
),

universo AS (
    SELECT
        split.producto_key,
        bucket_tienda.bucket,
        dp.ndf_id AS ndf_id_verdadero,
        UPPER(dn.laboratorio) = 'SANFER' AS es_positivo_sanfer
    FROM `scenic-firefly-473823-f7.precios_ml.producto_split_aportadores` AS split
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_producto` AS dp
        ON dp.producto_key = split.producto_key
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_ndf` AS dn
        ON dn.ndf_id = dp.ndf_id
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_tienda` AS dim_tienda
        ON dim_tienda.tienda_key = split.tienda_key
    INNER JOIN bucket_tienda
        ON bucket_tienda.tienda_slug = dim_tienda.tienda_slug
    WHERE split.split = 'calibracion'
),

candidato_top AS (
    SELECT
        producto_key,
        ndf_id AS ndf_id_candidato,
        distancia,
        margen,
        es_reciproco
        AND (atributos_producto.dosis_mg IS NULL OR atributos_ndf.dosis_mg IS NULL
             OR atributos_producto.dosis_mg = atributos_ndf.dosis_mg)
        AND (atributos_producto.volumen_ml IS NULL OR atributos_ndf.volumen_ml IS NULL
             OR atributos_producto.volumen_ml = atributos_ndf.volumen_ml)
        AND (atributos_producto.masa_g IS NULL OR atributos_ndf.masa_g IS NULL
             OR atributos_producto.masa_g = atributos_ndf.masa_g)
        AND (atributos_producto.piezas IS NULL OR atributos_ndf.piezas IS NULL
             OR atributos_producto.piezas = atributos_ndf.piezas) AS pasa_guardas
    FROM `scenic-firefly-473823-f7.precios_ml.int_candidatos_ndf`
    WHERE rank_desde_producto = 1
),

aceptados_por_distancia AS (
    SELECT
        candidato_top.margen,
        candidato_top.ndf_id_candidato = universo.ndf_id_verdadero AS top1_correcto
    FROM universo
    INNER JOIN candidato_top USING (producto_key)
    WHERE universo.es_positivo_sanfer
        AND candidato_top.ndf_id_candidato IS NOT NULL
        AND candidato_top.pasa_guardas
        AND (
            (universo.bucket = 'A' AND candidato_top.distancia <= 0.05)
            OR (universo.bucket = 'B' AND candidato_top.distancia <= 0.04)
        )
),

deltas AS (
    SELECT ROUND(delta, 4) AS delta_min_candidato
    FROM UNNEST(GENERATE_ARRAY(0.0, 0.03, 0.002)) AS delta
)

SELECT
    delta_min_candidato,
    COUNTIF(top1_correcto) AS n_correcto_total,
    COUNTIF(top1_correcto AND (margen IS NULL OR margen < delta_min_candidato))
        AS n_correcto_a_banda_gris,
    COUNTIF(NOT top1_correcto) AS n_incorrecto_total,
    COUNTIF(NOT top1_correcto AND (margen IS NULL OR margen < delta_min_candidato))
        AS n_incorrecto_a_banda_gris
FROM aceptados_por_distancia
CROSS JOIN deltas
GROUP BY delta_min_candidato
ORDER BY delta_min_candidato;

-- Paso 4: proyeccion de banda gris y cobertura sobre el universo COMPLETO
-- de int_candidatos_ndf (24,043 producto_key con algun vecino Sanfer en su
-- top-50, no solo el split de calibracion -en produccion no se sabe de
-- antemano cuales son positivos). tau_alto=0.05 (A) / 0.04 (B),
-- delta_min=0.008 fijos, sweep de tau_bajo por bucket. Resultado: 0.055 en
-- los dos buckets da banda gris 995 (A) + 1,006 (B) = 2,001 filas -en el
-- orden de 2,000 que pide el issue-, aceptado 710 (A) + 618 (B) = 1,328.
WITH bucket_tienda AS (
    SELECT tienda_key,
        CASE
            WHEN tienda_slug IN (
                'gi', 'klyns', 'isseg', 'aurrera', 'heb', 'farmatodo',
                'guadalajara', 'benavides', 'walmart'
            ) THEN 'A'
            ELSE 'B'
        END AS bucket
    FROM `scenic-firefly-473823-f7.precios_gold.dim_tienda`
),

candidato_top AS (
    SELECT
        bucket_tienda.bucket,
        c.ndf_id,
        c.distancia,
        (
            c.distancia <= IF(bucket_tienda.bucket = 'A', 0.05, 0.04)
            AND (c.margen IS NOT NULL AND c.margen >= 0.008)
            AND c.es_reciproco
            AND (c.atributos_producto.dosis_mg IS NULL OR c.atributos_ndf.dosis_mg IS NULL
                 OR c.atributos_producto.dosis_mg = c.atributos_ndf.dosis_mg)
            AND (c.atributos_producto.volumen_ml IS NULL OR c.atributos_ndf.volumen_ml IS NULL
                 OR c.atributos_producto.volumen_ml = c.atributos_ndf.volumen_ml)
            AND (c.atributos_producto.masa_g IS NULL OR c.atributos_ndf.masa_g IS NULL
                 OR c.atributos_producto.masa_g = c.atributos_ndf.masa_g)
            AND (c.atributos_producto.piezas IS NULL OR c.atributos_ndf.piezas IS NULL
                 OR c.atributos_producto.piezas = c.atributos_ndf.piezas)
        ) AS aceptado
    FROM `scenic-firefly-473823-f7.precios_ml.int_candidatos_ndf` AS c
    INNER JOIN bucket_tienda ON bucket_tienda.tienda_key = c.tienda_key
    WHERE c.rank_desde_producto = 1
),

tau_bajo_candidatos AS (
    SELECT ROUND(tau_bajo, 3) AS tau_bajo
    FROM UNNEST(GENERATE_ARRAY(0.05, 0.17, 0.005)) AS tau_bajo
)

SELECT
    candidato_top.bucket,
    tau_bajo_candidatos.tau_bajo,
    COUNTIF(candidato_top.aceptado) AS n_aceptado,
    COUNTIF(
        NOT candidato_top.aceptado
        AND candidato_top.distancia <= tau_bajo_candidatos.tau_bajo
    ) AS n_banda_gris,
    COUNTIF(candidato_top.distancia > tau_bajo_candidatos.tau_bajo) AS n_rechazado
FROM candidato_top
CROSS JOIN tau_bajo_candidatos
GROUP BY candidato_top.bucket, tau_bajo_candidatos.tau_bajo
ORDER BY candidato_top.bucket, tau_bajo_candidatos.tau_bajo;

-- Paso 5: cobertura de ndf_id, tau_bajo=0.055 fijo en los dos buckets.
-- Resultado: baseline (aportadores) 281 ndf_id / 1,389 (20.2%); con el
-- bucket aceptado, union total 382 ndf_id (27.5%; 52.0% contra el
-- denominador "alcanzable" de 734, ALD-82) -101 ndf_id nuevos que el
-- crosswalk no tenia.
WITH bucket_tienda AS (
    SELECT tienda_key,
        CASE
            WHEN tienda_slug IN (
                'gi', 'klyns', 'isseg', 'aurrera', 'heb', 'farmatodo',
                'guadalajara', 'benavides', 'walmart'
            ) THEN 'A'
            ELSE 'B'
        END AS bucket
    FROM `scenic-firefly-473823-f7.precios_gold.dim_tienda`
),

candidato_top AS (
    SELECT
        c.ndf_id,
        (
            c.distancia <= IF(bucket_tienda.bucket = 'A', 0.05, 0.04)
            AND (c.margen IS NOT NULL AND c.margen >= 0.008)
            AND c.es_reciproco
            AND (c.atributos_producto.dosis_mg IS NULL OR c.atributos_ndf.dosis_mg IS NULL
                 OR c.atributos_producto.dosis_mg = c.atributos_ndf.dosis_mg)
            AND (c.atributos_producto.volumen_ml IS NULL OR c.atributos_ndf.volumen_ml IS NULL
                 OR c.atributos_producto.volumen_ml = c.atributos_ndf.volumen_ml)
            AND (c.atributos_producto.masa_g IS NULL OR c.atributos_ndf.masa_g IS NULL
                 OR c.atributos_producto.masa_g = c.atributos_ndf.masa_g)
            AND (c.atributos_producto.piezas IS NULL OR c.atributos_ndf.piezas IS NULL
                 OR c.atributos_producto.piezas = c.atributos_ndf.piezas)
        ) AS aceptado
    FROM `scenic-firefly-473823-f7.precios_ml.int_candidatos_ndf` AS c
    INNER JOIN bucket_tienda ON bucket_tienda.tienda_key = c.tienda_key
    WHERE c.rank_desde_producto = 1
),

ndf_aceptados AS (
    SELECT DISTINCT ndf_id FROM candidato_top WHERE aceptado
),

aportadores_sanfer AS (
    SELECT DISTINCT dp.ndf_id
    FROM `scenic-firefly-473823-f7.precios_gold.dim_producto` AS dp
    INNER JOIN `scenic-firefly-473823-f7.precios_gold.dim_ndf` AS dn ON dn.ndf_id = dp.ndf_id
    WHERE dp.match_method = 'aportadores' AND UPPER(dn.laboratorio) = 'SANFER'
)

SELECT
    (SELECT COUNT(*) FROM `scenic-firefly-473823-f7.precios_ml.emb_ndf_sanfer`)
        AS total_ndf_sanfer,
    (SELECT COUNT(*) FROM aportadores_sanfer) AS baseline_ndf,
    (SELECT COUNT(*) FROM ndf_aceptados) AS vectorial_ndf,
    (SELECT COUNT(*) FROM aportadores_sanfer) + (
        SELECT COUNT(*) FROM ndf_aceptados
        WHERE ndf_id NOT IN (SELECT ndf_id FROM aportadores_sanfer)
    ) AS union_total;
